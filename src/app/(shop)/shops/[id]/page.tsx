...
import { Menu, ShoppingCart, ArrowLeft, MapPin, Star, Clock, Plus, Search, X } from "lucide-react";
    ...
    setCartItems((prevItems) => {
      ...
      if (existingItem) {
        ...
        return prevItems.map((item) =>
          item.id === product._id
            ? { ...item, quantity: item.inventory !== undefined ? Math.min(item.quantity + 1, item.inventory) : item.quantity + 1 }
            : item
        );
      } else {
  ...
  return (
    <section className="min-h-screen bg-gray-50 flex flex-col relative pb-20">
      ...
      <div className="fixed top-0 left-0 right-0 z-40 flex items-center justify-between p-4 bg-gradient-to-b from-black/60 to-transparent pointer-events-none transition-all">
        <button onClick={() => router.back()} className="pointer-events-auto w-10 h-10 bg-white/95 backdrop-blur-md rounded-full shadow-md text-gray-800 flex items-center justify-center hover:scale-105 transition active:scale-95">
          <ArrowLeft size={20} />
        </button>
        <div className="flex gap-3 pointer-events-auto">
          ...
          <button onClick={() => setOpenCartModal(true)} className="relative w-10 h-10 bg-white/95 backdrop-blur-md rounded-full shadow-md text-gray-800 flex items-center justify-center hover:scale-105 transition active:scale-95">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto bg-[#F9FAFB]">
          ...
          >
            <img 
              src={store?.data?.store?.avatar?.imageUrl || "/assets/placeholder.png"} 
          ...
          <div className="relative z-10 px-4 md:px-8 -mt-20">
            <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 p-5 md:p-8">
              <div className="flex flex-col md:flex-row gap-4 md:items-end md:justify-between">
                <div>
                  <h1 className="text-2xl md:text-4xl font-extrabold text-gray-900 tracking-tight">{store?.data?.store?.name}</h1>
                ...
                <div className="flex gap-2">
                   <div className="bg-green-50 text-green-700 px-3 py-1.5 rounded-2xl text-xs font-bold flex items-center gap-1 border border-green-100 shadow-sm"><Star size={13} className="fill-green-700"/> 4.8 Excellent</div>
                   <div className="bg-gray-50 text-gray-700 px-3 py-1.5 rounded-2xl text-xs font-bold flex items-center gap-1 border border-gray-100 shadow-sm"><Clock size={13}/> 20-35 min</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="px-4 md:px-8 py-6 max-w-7xl mx-auto">
            {areProductsLoading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {[1, 2, 3, 4, 5, 6].map(i => (
                  <div key={i} className="bg-white rounded-2xl p-4 h-36 animate-pulse border border-gray-100 flex gap-4">
                    <div className="flex-1 space-y-3 py-2"><div className="h-4 bg-gray-100 rounded w-3/4"></div><div className="h-3 bg-gray-100 rounded w-full"></div><div className="h-3 bg-gray-100 rounded w-1/2"></div></div>
              ...
              <>
                <h2 className="text-xl font-bold text-gray-900 mb-6 px-1">
                  {selectedCategory ? getCategoryName(selectedCategory) : "Popular items"}
                </h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {products.map(product => (
                    <div key={product._id} className="bg-white rounded-2xl p-4 flex gap-4 border border-gray-100 hover:border-blue-100 hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-300 group cursor-pointer" onClick={() => setSelectedProduct(product)}>
                      <div className="flex-1 min-w-0 flex flex-col">
                        <h3 className="font-bold text-gray-900 text-[15px] leading-snug line-clamp-2 group-hover:text-[var(--color-blue-primary)] transition-colors">{product.title}</h3>
                        <p className="text-gray-500 text-[13px] mt-1.5 line-clamp-2 leading-relaxed">{product.description}</p>
                      ...
                      <div className="w-32 h-32 flex-shrink-0 bg-gray-50 rounded-xl overflow-hidden relative border border-gray-100">
                        {product.image?.imageUrl ? (
                          <img src={product.image.imageUrl} alt={product.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        ) : (
                ...
                {products.length === 0 && (
                  <div className="text-center py-20 bg-white rounded-3xl border border-gray-100 mt-4 shadow-sm">
                    ...
                    <p className="text-gray-500 text-sm">
                      {selectedCategory ? `We couldn't find any products in ${getCategoryName(selectedCategory)}.` : "This store hasn't added any products yet."}
                    </p>
                  </div>
                )}
              </>
          ...
          <DialogContent className="sm:max-w-md p-0 overflow-hidden border-none rounded-2xl shadow-2xl bg-white">
            <div className="relative h-64 bg-gray-50 w-full">
              {selectedProduct.image?.imageUrl ? (
                <img src={selectedProduct.image.imageUrl} alt={selectedProduct.title} className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="p-6">
              <div className="flex justify-between items-start gap-4 mb-4">
                ...
                <span className="font-extrabold text-[var(--color-blue-primary)] text-xl shrink-0">
                  {new Intl.NumberFormat("en-NG", { style: "currency", currency: "NGN", maximumFractionDigits: 0 }).format(selectedProduct.price)}
                </span>
              </div>
              <DialogDescription className="text-gray-600 text-[15px] leading-relaxed mb-8">
                {selectedProduct.description || "Fresh and delicious, order now."}
              </DialogDescription>
              {(!selectedProduct.isAvailable || selectedProduct.inventory <= 0) ? (
