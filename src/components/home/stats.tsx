...
const Stats = ({ country }: { country: Country }) => {
  const { coverStat } = contentFor(country);
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-20">
        <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-white/10">
          ...
          <div className="flex flex-col items-center sm:items-start gap-1 py-8 sm:py-0 sm:px-10">
            ...
            <p className="font-black text-5xl md:text-6xl text-white tracking-tight">
              <CountUp endValue={coverStat.value} suffix={coverStat.suffix} />
            </p>
            <p className="text-sm font-medium text-white/50 mt-1">{coverStat.label}</p>
          </div>
        </div>
      </div>
    </section>
